...
class LocalPgFlywayEndToEndSpec extends BaseIntegrationSpec {
    ...
    def "local-pg uruchamia pełne migracje Flyway i tworzy kompletny schemat"() {
        ...
        then: "aplikacja działa na profilu local-pg z Flyway włączonym i bez Hibernate DDL"
        dbInfo.activeProfiles.contains('local-pg')
        dbInfo.flywayEnabled == 'true'
        dbInfo.jpaDdlAuto == 'none'
        (dbInfo.datasourceUrl as String).contains('jdbc:postgresql://localhost:5432/smartfin_test')
        dbInfo.datasourceUsername == 'finuser'
    ...
    def "local-pg end-to-end: Flyway admin login, publiczna rejestracja usera, upload admina, odczyt i update ownera przez REST"() {
        given: "admin pochodzi z pełnych migracji Flyway, a owner jest tworzony przez publiczne API"
        repo.deleteAll()
        String adminToken = loginAndGetToken('admin', 'admin123')
        String ownerUsername = "local_pg_e2e_user_${System.nanoTime()}"
        String ownerPassword = 'local-pg-e2e-pass-123'
        registerUser(ownerUsername, ownerPassword)
        String userToken = loginAndGetToken(ownerUsername, ownerPassword)
        String originalId = "LOCAL-PG-E2E-${System.nanoTime()}"
        def csv = "id,amount,currency,category,description,date\n${originalId},120.00,PLN,Zakupy,Myszka local-pg e2e,2026-04-12\n".getBytes('UTF-8')
        def file = new MockMultipartFile('file', 'local-pg-e2e.csv', 'text/csv', csv)
        ...
        mvc.perform(get("/api/transactions/${saved[0].dbId}")
                ...
                .andExpect(jsonPath('$.description').value('Myszka local-pg e2e'))
        ...
        def updatePayload = JsonOutput.toJson([
                ...
                currency   : 'PLN',
                category   : 'Zakupy',
                description: 'Myszka local-pg e2e po PUT'
        ])
        ...
        mvc.perform(put("/api/transactions/${saved[0].dbId}")
                ...
                .andExpect(jsonPath('$.description').value('Myszka local-pg e2e po PUT'))
        ...
        businessRows[0].description == 'Myszka local-pg e2e po PUT'
        ...
        auditRows[0].description == 'Myszka local-pg e2e'
        (auditRows[1].amount as BigDecimal) == 135.00G
        auditRows[1].description == 'Myszka local-pg e2e po PUT'
    }
    ...
    private static List<Integer> expectedFlywayVersions() {
